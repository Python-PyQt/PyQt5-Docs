.. sip:method-description::
    :status: todo
    :pysig: 5f31f76a3929e7e5a35e8b8441100c11
...
The only way you can use a document type that was created this way, is in combination with the :sip:ref:`~PyQt5.QtXml.QDomImplementation.createDocument` function to create a :sip:ref:`~PyQt5.QtXml.QDomDocument` with this document type.

In the DOM specification, this is the only way to create a non-null document. For historical reasons, Qt also allows to create the document using the default empty constructor. The resulting document is null, but becomes non-null when a factory function, for example :sip:ref:`~PyQt5.QtXml.QDomDocument.createElement`, is called. The document also becomes non-null when setContent() is called.

.. seealso:: :sip:ref:`~PyQt5.QtXml.QDomImplementation.createDocument`.

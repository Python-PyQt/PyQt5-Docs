...
Appends the floating point number *d* to the stream, creating a CBOR 64-bit Double-Precision Floating Point value. :sip:ref:`~PyQt5.QtCore.QCborStreamWriter` always appends the number as-is, performing no check for whether the number is the canonical form for NaN, an infinite, whether it is denormal or if it could be written with a shorter format.
...
.. seealso:: QCborStreamReader::isDouble()QCborStreamReader::toDouble().

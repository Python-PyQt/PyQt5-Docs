.. sip:enum-description::
    :status: todo
    :digest: 229ba6ccbdb5d36d84967e1d9ac25ce4

This enum contains the different clock types that :sip:ref:`~PyQt5.QtCore.QElapsedTimer` may use.

:sip:ref:`~PyQt5.QtCore.QElapsedTimer` will always use the same clock type in a particular machine, so this value will not change during the lifetime of a program. It is provided so that :sip:ref:`~PyQt5.QtCore.QElapsedTimer` can be used with other non-Qt implementations, to guarantee that the same reference clock is being used.
...
This is the only non-monotonic clock that :sip:ref:`~PyQt5.QtCore.QElapsedTimer` may use.
...
The :sip:ref:`~PyQt5.QtCore.QElapsedTimer.ClockType.TickCounter` clock type is the only clock type that may overflow. Windows Vista and Windows Server 2008 support the extended 64-bit tick counter, which allows avoiding the overflow.
...
This clock type is based on the absolute time presented by Mach kernels, such as that found on `macOS <https://doc.qt.io/qt-5/qtwebengine-platform-notes.html#macos>`_. This clock type is presented separately from :sip:ref:`~PyQt5.QtCore.QElapsedTimer.ClockType.MonotonicClock` since `macOS <https://doc.qt.io/qt-5/qtwebengine-platform-notes.html#macos>`_ and iOS are also Unix systems and may support a POSIX monotonic clock with values differing from the Mach absolute time.
...
This clock uses the Windows functions ``QueryPerformanceCounter`` and ``QueryPerformanceFrequency`` to access the system's high-precision performance counter. Since this counter may not be available on all systems, :sip:ref:`~PyQt5.QtCore.QElapsedTimer` will fall back to the ``\ :sip:ref:`~PyQt5.QtCore.QElapsedTimer.ClockType.TickCounter``` clock automatically, if this clock cannot be used.

This clock is monotonic and does not overflow.

.. seealso:: :sip:ref:`~PyQt5.QtCore.QElapsedTimer.clockType`, :sip:ref:`~PyQt5.QtCore.QElapsedTimer.isMonotonic`.

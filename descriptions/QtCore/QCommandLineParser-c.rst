.. sip:class-description::
    ...
    :digest: 33e261b2a2407d398bad00b431da79e5

The :sip:ref:`~PyQt5.QtCore.QCommandLineParser` class provides a means for handling the command line options.

:sip:ref:`~PyQt5.QtCore.QCoreApplication` provides the command-line arguments as a simple list of strings. :sip:ref:`~PyQt5.QtCore.QCommandLineParser` provides the ability to define a set of options, parse the command-line arguments, and store which options have actually been used, as well as option values.
...
If your compiler supports the C++11 standard, the three :sip:ref:`~PyQt5.QtCore.QCommandLineParser.addOption` calls in the above example can be simplified:

.. literalinclude:: ../../../snippets/qtbase-src-corelib-doc-snippets-code-src_corelib_tools_qcommandlineparser_main.py
    :lines: 100-111

Known limitation: the parsing of Qt options inside :sip:ref:`~PyQt5.QtCore.QCoreApplication` and subclasses happens before :sip:ref:`~PyQt5.QtCore.QCommandLineParser` exists, so it can't take it into account. This means any option value that looks like a builtin Qt option, will be treated by :sip:ref:`~PyQt5.QtCore.QCoreApplication` as a builtin Qt option. Example: ``--profile -reverse`` will lead to :sip:ref:`~PyQt5.QtGui.QGuiApplication` seeing the -reverse option set, and removing it from :sip:ref:`~PyQt5.QtCore.QCoreApplication.arguments` before :sip:ref:`~PyQt5.QtCore.QCommandLineParser` defines the ``profile`` option and parses the command line.
...
It is then advisable to introduce a function to do the command line parsing which takes a struct or class receiving the option values returning an enumeration representing the result. The dnslookup example of the :sip:ref:`~PyQt5.QtNetwork` module illustrates this:
...
On Windows, :sip:ref:`~PyQt5.QtCore.QCommandLineParser` uses message boxes to display usage information and errors if no console window can be obtained.
...
.. seealso:: :sip:ref:`~PyQt5.QtCore.QCommandLineOption`, :sip:ref:`~PyQt5.QtCore.QCoreApplication`.

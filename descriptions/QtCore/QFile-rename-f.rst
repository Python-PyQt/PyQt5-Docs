...
Renames the file currently specified by :sip:ref:`~PyQt5.QtCore.QFile.fileName` to *newName*. Returns ``true`` if successful; otherwise returns ``false``.

If a file with the name *newName* already exists,  returns ``false`` (i.e., :sip:ref:`~PyQt5.QtCore.QFile` will not overwrite it).

...
If *directory* is not specified, the current directory is used (i.e., as :sip:ref:`~PyQt5.QtCore.QDir.currentPath`).
...
Usually, it is better to use the (const :sip:ref:`~PyQt5.QtCore.QLocale` &, const QString &, const QString &, const QString &, const QString &) function instead, because it uses :sip:ref:`~PyQt5.QtCore.QLocale.uiLanguages` and not simply the locale name, which refers to the formatting of dates and numbers and not necessarily the UI language.

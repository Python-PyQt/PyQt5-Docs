...
The :sip:ref:`~PyQt5.QtRemoteObjects` module contains classes that implement an
inter-process communication mechanism for sharing the API of a
:sip:ref:`~PyQt5.QtCore.QObject` (i.e. its properties, signals and slots)
...
the same connections and behavior of :sip:ref:`~PyQt5.QtCore.QObject`\s, which
makes it as easy to use as any other locally implemented
:sip:ref:`~PyQt5.QtCore.QObject`.  Everything needed for the replica to look
like the source object is handled behind the scenes.

In Qt the API of the :sip:ref:`~PyQt5.QtCore.QObject` can be defined either
...
C++ code is then compiled and linked with the rest of the application.  PyQt5
does not support static definitions.

A dynamic definition uses a conventionally written
:sip:ref:`~PyQt5.QtCore.QObject`.  It is then introspected to determine the API
...
definition of the :sip:ref:`~PyQt5.QtCore.QObject`.
...
Python server must ensure that the :sip:ref:`~PyQt5.QtCore.QMetaObject` of the
:sip:ref:`~PyQt5.QtCore.QObject` matches that defined in the ``.rep`` file.

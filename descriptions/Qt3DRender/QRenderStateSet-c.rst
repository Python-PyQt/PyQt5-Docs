...
The :sip:ref:`~PyQt5.Qt3DRender.QRenderStateSet` FrameGraph node offers a way of specifying a set of QRenderState objects to be applied during the execution of a framegraph branch.

States set on a :sip:ref:`~PyQt5.Qt3DRender.QRenderStateSet` are set globally, contrary to the per-material states that can be set on a QRenderPass. By default, an empty :sip:ref:`~PyQt5.Qt3DRender.QRenderStateSet` will result in all render states being disabled when executed. Adding a QRenderState state explicitly enables that render state at runtime.

The `RenderStateSet <https://doc.qt.io/qt-5/qml-qt3d-render-renderstateset.html>`_ is enabled when added to the active frame graph:

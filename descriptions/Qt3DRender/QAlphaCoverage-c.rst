...
A :sip:ref:`~PyQt5.Qt3DRender.QAlphaCoverage` class enables alpha-to-coverage multisampling mode. When enabled, the fragment alpha value is used as a coverage for the sample and combined with fragment coverage value. :sip:ref:`~PyQt5.Qt3DRender.QAlphaCoverage` does nothing if multisampling is disabled. Alpha-to-coverage is most useful when order independent blending is required, for example when rendering leaves, grass and other rich vegetation.
...
.. seealso:: :sip:ref:`~PyQt5.Qt3DRender.QMultiSampleAntiAliasing`.

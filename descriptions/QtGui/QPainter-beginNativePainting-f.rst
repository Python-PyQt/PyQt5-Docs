...
Flushes the painting pipeline and prepares for the user issuing commands directly to the underlying graphics context. Must be followed by a call to :sip:ref:`~PyQt5.QtGui.QPainter.endNativePainting`.
...
If, for example, the OpenGL polygon mode is changed by the user inside a beginNativePaint()/\ :sip:ref:`~PyQt5.QtGui.QPainter.endNativePainting` block, it will not be reset to the default state by :sip:ref:`~PyQt5.QtGui.QPainter.endNativePainting`. Here is an example that shows intermixing of painter commands and raw OpenGL commands:

.. literalinclude:: ../../../snippets/qtbase-src-gui-doc-snippets-code-src_gui_painting_qpainter.py
    :lines: 258-270

.. seealso:: :sip:ref:`~PyQt5.QtGui.QPainter.endNativePainting`.

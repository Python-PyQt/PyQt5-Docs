...
Schedules a :sip:ref:`~PyQt5.QtCore.QEvent.Type.UpdateRequest` event to be delivered to this window.
...
Subclasses of :sip:ref:`~PyQt5.QtGui.QWindow` should reimplement :sip:ref:`~PyQt5.QtGui.QWindow.event`, intercept the event and call the application's rendering code, then call the base class implementation.

**Note:** The subclass' reimplementation of :sip:ref:`~PyQt5.QtGui.QWindow.event` must invoke the base class implementation, unless it is absolutely sure that the event does not need to be handled by the base class. For example, the default implementation of this function relies on :sip:ref:`~PyQt5.QtCore.QEvent.Type.Timer` events. Filtering them away would therefore break the delivery of the update events.

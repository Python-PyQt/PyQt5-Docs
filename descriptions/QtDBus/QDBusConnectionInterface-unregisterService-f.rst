.. sip:method-description::
    :status: todo
    :pysig: a7ec19e7317ffc378dac8fb5abd6b4b0
    :realsig: (const QString&)
    :digest: d3c270905799ac80f98321810b161981

Releases the claim on the bus service name *serviceName*, that had been previously registered with :sip:ref:`~PyQt5.QtDBus.QDBusConnectionInterface.registerService`. If this application had ownership of the name, it will be released for other applications to claim. If it only had the name queued, it gives up its position in the queue.

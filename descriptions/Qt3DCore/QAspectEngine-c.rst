...
:sip:ref:`~PyQt5.Qt3DCore.QAbstractAspect` subclasses can be registered by calling Qt3DCore::QAspectEngine::registerAspect() which will take care of registering the aspect and in turn that will call Qt3DCore::QAbstractAspect::onRegistered();

The simulation loop is launched as soon as a root :sip:ref:`~PyQt5.Qt3DCore.QEntity` is set on the :sip:ref:`~PyQt5.Qt3DCore.QAspectEngine`. This is followed by a call to onEngineStartup() on each aspect so that they can start their simulation work.

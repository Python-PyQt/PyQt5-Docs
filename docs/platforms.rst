...
With v5.5.0 the macOS version of Qt is built so that the Qt frameworks are
built using the *rpath* mechanism.  Unfortunately this causes problems for
...
:sip:ref:`QtGui` module.  However the problem can arise with other plugins.
The solution is the set the :envvar:`DYLD_FRAMEWORK_PATH` environment variable
to the name of the ``lib`` directory in your Qt installation, i.e. the
directory containing the Qt frameworks.

...
In previous versions PyQt5 did not perform overflow checking when converting
Python ``int`` objects to C++ integer types.  The behaviour when overflow
occurred was undefined.

This version performs overflow checking and will raise an appropriate exception
...
Importing the :sip:ref:`~PyQt5.sip` Module
******************************************

In previous versions PyQt5 used the copy of the :py:mod:`~PyQt5.sip` module
...
but this will only work if another :mod:`PyQt5` module is imported first.  For
...
Enforcement of :sip:ref:`~PyQt5.QtCore.pyqtSlot` Signatures
***********************************************************

In previous versions if a signal was connected to a method that was decorated
by :sip:ref:`~PyQt5.QtCore.pyqtSlot`, and the signatures of the signal and slot
...
Conversion of Latin-1 Strings to :sip:ref:`~PyQt5.QtCore.QByteArray`
********************************************************************

This version removes the automatic conversion of a Latin-1 encoded string when
a :sip:ref:`~PyQt5.QtCore.QByteArray` is expected.  It was deprecated in PyQt
v5.4.
...
:sip:ref:`sys.excepthook`.  The default exception hook would then display the
...
In previous versions, when a signal was emitted to a Python slot that was not
decorated with :sip:ref:`~PyQt5.QtCore.pyqtSlot`, it would not check that the
underlying C++ receiver instance still existed.  This matched the PyQt v4
behaviour at the time that PyQt v5.0 was released, but doesn't reflect the
standard C++ behaviour.

The lack of a check meant that an object could connect its
:sip:ref:`~PyQt5.QtCore.QObject.destroyed` signal to itself so that it could
...
:sip:ref:`~PyQt5.QtCore.QObject.connect` which allows the check to be
suppressed on a per connection basis.

:orphan:

.. sip:class:: PyQt5.QtWidgets.QSystemTrayIcon
    :inherits: :sip:ref:`~PyQt5.QtCore.QObject`
    :description: QtWidgets/QSystemTrayIcon-c.rst

    .. sip:enum:: PyQt5.QtWidgets.QSystemTrayIcon.ActivationReason
        :description: QtWidgets/QSystemTrayIcon-ActivationReason-e.rst

        .. sip:enum-member:: PyQt5.QtWidgets.QSystemTrayIcon.ActivationReason.Context
            :description: QtWidgets/QSystemTrayIcon-ActivationReason-Context-v.rst

        .. sip:enum-member:: PyQt5.QtWidgets.QSystemTrayIcon.ActivationReason.DoubleClick
            :description: QtWidgets/QSystemTrayIcon-ActivationReason-DoubleClick-v.rst

        .. sip:enum-member:: PyQt5.QtWidgets.QSystemTrayIcon.ActivationReason.MiddleClick
            :description: QtWidgets/QSystemTrayIcon-ActivationReason-MiddleClick-v.rst

        .. sip:enum-member:: PyQt5.QtWidgets.QSystemTrayIcon.ActivationReason.Trigger
            :description: QtWidgets/QSystemTrayIcon-ActivationReason-Trigger-v.rst

        .. sip:enum-member:: PyQt5.QtWidgets.QSystemTrayIcon.ActivationReason.Unknown
            :description: QtWidgets/QSystemTrayIcon-ActivationReason-Unknown-v.rst

    .. sip:enum:: PyQt5.QtWidgets.QSystemTrayIcon.MessageIcon
        :description: QtWidgets/QSystemTrayIcon-MessageIcon-e.rst

        .. sip:enum-member:: PyQt5.QtWidgets.QSystemTrayIcon.MessageIcon.Critical
            :description: QtWidgets/QSystemTrayIcon-MessageIcon-Critical-v.rst

        .. sip:enum-member:: PyQt5.QtWidgets.QSystemTrayIcon.MessageIcon.Information
            :description: QtWidgets/QSystemTrayIcon-MessageIcon-Information-v.rst

        .. sip:enum-member:: PyQt5.QtWidgets.QSystemTrayIcon.MessageIcon.NoIcon
            :description: QtWidgets/QSystemTrayIcon-MessageIcon-NoIcon-v.rst

        .. sip:enum-member:: PyQt5.QtWidgets.QSystemTrayIcon.MessageIcon.Warning
            :description: QtWidgets/QSystemTrayIcon-MessageIcon-Warning-v.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.__init__
        :args:
            parent: :sip:ref:`~PyQt5.QtCore.QObject` = None
        :description: QtWidgets/QSystemTrayIcon-__init__-f.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.__init__
        :args:
            :sip:ref:`~PyQt5.QtGui.QIcon`
            parent: :sip:ref:`~PyQt5.QtCore.QObject` = None
        :description: QtWidgets/QSystemTrayIcon-__init__-f-1.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.contextMenu
        :returns:
            :sip:ref:`~PyQt5.QtWidgets.QMenu`
        :description: QtWidgets/QSystemTrayIcon-contextMenu-f.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.event
        :args:
            :sip:ref:`~PyQt5.QtCore.QEvent`
        :returns:
            bool
        :description: QtWidgets/QSystemTrayIcon-event-f.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.geometry
        :returns:
            :sip:ref:`~PyQt5.QtCore.QRect`
        :description: QtWidgets/QSystemTrayIcon-geometry-f.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.hide
        :description: QtWidgets/QSystemTrayIcon-hide-f.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.icon
        :returns:
            :sip:ref:`~PyQt5.QtGui.QIcon`
        :description: QtWidgets/QSystemTrayIcon-icon-f.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.isSystemTrayAvailable
        :returns:
            bool
        :static:
        :description: QtWidgets/QSystemTrayIcon-isSystemTrayAvailable-f.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.isVisible
        :returns:
            bool
        :description: QtWidgets/QSystemTrayIcon-isVisible-f.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.setContextMenu
        :args:
            :sip:ref:`~PyQt5.QtWidgets.QMenu`
        :description: QtWidgets/QSystemTrayIcon-setContextMenu-f.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.setIcon
        :args:
            :sip:ref:`~PyQt5.QtGui.QIcon`
        :description: QtWidgets/QSystemTrayIcon-setIcon-f.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.setToolTip
        :args:
            str
        :description: QtWidgets/QSystemTrayIcon-setToolTip-f.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.setVisible
        :args:
            bool
        :description: QtWidgets/QSystemTrayIcon-setVisible-f.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.show
        :description: QtWidgets/QSystemTrayIcon-show-f.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.showMessage
        :args:
            str
            str
            icon: :sip:ref:`~PyQt5.QtWidgets.QSystemTrayIcon.MessageIcon` = :sip:ref:`~PyQt5.QtWidgets.QSystemTrayIcon.MessageIcon.Information`
            msecs: int = 10000
        :description: QtWidgets/QSystemTrayIcon-showMessage-f.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.showMessage
        :args:
            str
            str
            :sip:ref:`~PyQt5.QtGui.QIcon`
            msecs: int = 10000
        :description: QtWidgets/QSystemTrayIcon-showMessage-f-1.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.supportsMessages
        :returns:
            bool
        :static:
        :description: QtWidgets/QSystemTrayIcon-supportsMessages-f.rst

    .. sip:method:: PyQt5.QtWidgets.QSystemTrayIcon.toolTip
        :returns:
            str
        :description: QtWidgets/QSystemTrayIcon-toolTip-f.rst

    .. sip:signal:: PyQt5.QtWidgets.QSystemTrayIcon.activated
        :args:
            :sip:ref:`~PyQt5.QtWidgets.QSystemTrayIcon.ActivationReason`
        :description: QtWidgets/QSystemTrayIcon-activated-s.rst

    .. sip:signal:: PyQt5.QtWidgets.QSystemTrayIcon.messageClicked
        :description: QtWidgets/QSystemTrayIcon-messageClicked-s.rst

:orphan:

.. sip:class:: PyQt5.QtGui.QImageReader
    :description: QtGui/QImageReader-c.rst

    .. sip:enum:: PyQt5.QtGui.QImageReader.ImageReaderError
        :description: QtGui/QImageReader-ImageReaderError-e.rst

        .. sip:enum-member:: PyQt5.QtGui.QImageReader.ImageReaderError.DeviceError
            :description: QtGui/QImageReader-ImageReaderError-DeviceError-v.rst

        .. sip:enum-member:: PyQt5.QtGui.QImageReader.ImageReaderError.FileNotFoundError
            :description: QtGui/QImageReader-ImageReaderError-FileNotFoundError-v.rst

        .. sip:enum-member:: PyQt5.QtGui.QImageReader.ImageReaderError.InvalidDataError
            :description: QtGui/QImageReader-ImageReaderError-InvalidDataError-v.rst

        .. sip:enum-member:: PyQt5.QtGui.QImageReader.ImageReaderError.UnknownError
            :description: QtGui/QImageReader-ImageReaderError-UnknownError-v.rst

        .. sip:enum-member:: PyQt5.QtGui.QImageReader.ImageReaderError.UnsupportedFormatError
            :description: QtGui/QImageReader-ImageReaderError-UnsupportedFormatError-v.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.__init__
        :description: QtGui/QImageReader-__init__-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.__init__
        :args:
            :sip:ref:`~PyQt5.QtCore.QIODevice`
            format: Union[:sip:ref:`~PyQt5.QtCore.QByteArray`, bytes, bytearray] = QByteArray()
        :description: QtGui/QImageReader-__init__-f-1.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.__init__
        :args:
            str
            format: Union[:sip:ref:`~PyQt5.QtCore.QByteArray`, bytes, bytearray] = QByteArray()
        :description: QtGui/QImageReader-__init__-f-2.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.autoDetectImageFormat
        :returns:
            bool
        :description: QtGui/QImageReader-autoDetectImageFormat-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.autoTransform
        :returns:
            bool
        :description: QtGui/QImageReader-autoTransform-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.backgroundColor
        :returns:
            :sip:ref:`~PyQt5.QtGui.QColor`
        :description: QtGui/QImageReader-backgroundColor-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.canRead
        :returns:
            bool
        :description: QtGui/QImageReader-canRead-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.clipRect
        :returns:
            :sip:ref:`~PyQt5.QtCore.QRect`
        :description: QtGui/QImageReader-clipRect-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.currentImageNumber
        :returns:
            int
        :description: QtGui/QImageReader-currentImageNumber-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.currentImageRect
        :returns:
            :sip:ref:`~PyQt5.QtCore.QRect`
        :description: QtGui/QImageReader-currentImageRect-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.decideFormatFromContent
        :returns:
            bool
        :description: QtGui/QImageReader-decideFormatFromContent-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.device
        :returns:
            :sip:ref:`~PyQt5.QtCore.QIODevice`
        :description: QtGui/QImageReader-device-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.error
        :returns:
            :sip:ref:`~PyQt5.QtGui.QImageReader.ImageReaderError`
        :description: QtGui/QImageReader-error-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.errorString
        :returns:
            str
        :description: QtGui/QImageReader-errorString-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.fileName
        :returns:
            str
        :description: QtGui/QImageReader-fileName-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.format
        :returns:
            :sip:ref:`~PyQt5.QtCore.QByteArray`
        :description: QtGui/QImageReader-format-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.gamma
        :returns:
            float
        :description: QtGui/QImageReader-gamma-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.imageCount
        :returns:
            int
        :description: QtGui/QImageReader-imageCount-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.imageFormat
        :returns:
            :sip:ref:`~PyQt5.QtGui.QImage.Format`
        :description: QtGui/QImageReader-imageFormat-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.imageFormat
        :args:
            str
        :returns:
            :sip:ref:`~PyQt5.QtCore.QByteArray`
        :static:
        :description: QtGui/QImageReader-imageFormat-f-1.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.imageFormat
        :args:
            :sip:ref:`~PyQt5.QtCore.QIODevice`
        :returns:
            :sip:ref:`~PyQt5.QtCore.QByteArray`
        :static:
        :description: QtGui/QImageReader-imageFormat-f-2.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.imageFormatsForMimeType
        :args:
            Union[:sip:ref:`~PyQt5.QtCore.QByteArray`, bytes, bytearray]
        :returns:
            List[:sip:ref:`~PyQt5.QtCore.QByteArray`]
        :static:
        :description: QtGui/QImageReader-imageFormatsForMimeType-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.jumpToImage
        :args:
            int
        :returns:
            bool
        :description: QtGui/QImageReader-jumpToImage-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.jumpToNextImage
        :returns:
            bool
        :description: QtGui/QImageReader-jumpToNextImage-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.loopCount
        :returns:
            int
        :description: QtGui/QImageReader-loopCount-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.nextImageDelay
        :returns:
            int
        :description: QtGui/QImageReader-nextImageDelay-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.quality
        :returns:
            int
        :description: QtGui/QImageReader-quality-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.read
        :returns:
            :sip:ref:`~PyQt5.QtGui.QImage`
        :description: QtGui/QImageReader-read-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.read
        :args:
            :sip:ref:`~PyQt5.QtGui.QImage`
        :returns:
            bool
        :description: QtGui/QImageReader-read-f-1.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.scaledClipRect
        :returns:
            :sip:ref:`~PyQt5.QtCore.QRect`
        :description: QtGui/QImageReader-scaledClipRect-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.scaledSize
        :returns:
            :sip:ref:`~PyQt5.QtCore.QSize`
        :description: QtGui/QImageReader-scaledSize-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.setAutoDetectImageFormat
        :args:
            bool
        :description: QtGui/QImageReader-setAutoDetectImageFormat-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.setAutoTransform
        :args:
            bool
        :description: QtGui/QImageReader-setAutoTransform-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.setBackgroundColor
        :args:
            Union[:sip:ref:`~PyQt5.QtGui.QColor`, :sip:ref:`~PyQt5.QtCore.Qt.GlobalColor`, :sip:ref:`~PyQt5.QtGui.QGradient`]
        :description: QtGui/QImageReader-setBackgroundColor-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.setClipRect
        :args:
            :sip:ref:`~PyQt5.QtCore.QRect`
        :description: QtGui/QImageReader-setClipRect-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.setDecideFormatFromContent
        :args:
            bool
        :description: QtGui/QImageReader-setDecideFormatFromContent-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.setDevice
        :args:
            :sip:ref:`~PyQt5.QtCore.QIODevice`
        :description: QtGui/QImageReader-setDevice-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.setFileName
        :args:
            str
        :description: QtGui/QImageReader-setFileName-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.setFormat
        :args:
            Union[:sip:ref:`~PyQt5.QtCore.QByteArray`, bytes, bytearray]
        :description: QtGui/QImageReader-setFormat-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.setGamma
        :args:
            float
        :description: QtGui/QImageReader-setGamma-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.setQuality
        :args:
            int
        :description: QtGui/QImageReader-setQuality-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.setScaledClipRect
        :args:
            :sip:ref:`~PyQt5.QtCore.QRect`
        :description: QtGui/QImageReader-setScaledClipRect-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.setScaledSize
        :args:
            :sip:ref:`~PyQt5.QtCore.QSize`
        :description: QtGui/QImageReader-setScaledSize-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.size
        :returns:
            :sip:ref:`~PyQt5.QtCore.QSize`
        :description: QtGui/QImageReader-size-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.subType
        :returns:
            :sip:ref:`~PyQt5.QtCore.QByteArray`
        :description: QtGui/QImageReader-subType-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.supportedImageFormats
        :returns:
            List[:sip:ref:`~PyQt5.QtCore.QByteArray`]
        :static:
        :description: QtGui/QImageReader-supportedImageFormats-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.supportedMimeTypes
        :returns:
            List[:sip:ref:`~PyQt5.QtCore.QByteArray`]
        :static:
        :description: QtGui/QImageReader-supportedMimeTypes-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.supportedSubTypes
        :returns:
            List[:sip:ref:`~PyQt5.QtCore.QByteArray`]
        :description: QtGui/QImageReader-supportedSubTypes-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.supportsAnimation
        :returns:
            bool
        :description: QtGui/QImageReader-supportsAnimation-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.supportsOption
        :args:
            :sip:ref:`~PyQt5.QtGui.QImageIOHandler.ImageOption`
        :returns:
            bool
        :description: QtGui/QImageReader-supportsOption-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.text
        :args:
            str
        :returns:
            str
        :description: QtGui/QImageReader-text-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.textKeys
        :returns:
            List[str]
        :description: QtGui/QImageReader-textKeys-f.rst

    .. sip:method:: PyQt5.QtGui.QImageReader.transformation
        :returns:
            :sip:ref:`~PyQt5.QtGui.QImageIOHandler.Transformations`
        :description: QtGui/QImageReader-transformation-f.rst

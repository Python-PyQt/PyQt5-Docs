:orphan:

.. sip:class:: PyQt5.QtGui.QTextLine
    :description: QtGui/QTextLine-c.rst

    .. sip:enum:: PyQt5.QtGui.QTextLine.CursorPosition
        :description: QtGui/QTextLine-CursorPosition-e.rst

        .. sip:enum-member:: PyQt5.QtGui.QTextLine.CursorPosition.CursorBetweenCharacters
            :description: QtGui/QTextLine-CursorPosition-CursorBetweenCharacters-v.rst

        .. sip:enum-member:: PyQt5.QtGui.QTextLine.CursorPosition.CursorOnCharacter
            :description: QtGui/QTextLine-CursorPosition-CursorOnCharacter-v.rst

    .. sip:enum:: PyQt5.QtGui.QTextLine.Edge
        :description: QtGui/QTextLine-Edge-e.rst

        .. sip:enum-member:: PyQt5.QtGui.QTextLine.Edge.Leading
            :description: QtGui/QTextLine-Edge-Leading-v.rst

        .. sip:enum-member:: PyQt5.QtGui.QTextLine.Edge.Trailing
            :description: QtGui/QTextLine-Edge-Trailing-v.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.__init__
        :description: QtGui/QTextLine-__init__-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.__init__
        :args:
            :sip:ref:`~PyQt5.QtGui.QTextLine`
        :description: QtGui/QTextLine-__init__-f-1.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.ascent
        :returns:
            float
        :description: QtGui/QTextLine-ascent-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.cursorToX
        :args:
            int
            edge: :sip:ref:`~PyQt5.QtGui.QTextLine.Edge` = :sip:ref:`~PyQt5.QtGui.QTextLine.Edge.Leading`
        :returns:
            float
            int
        :description: QtGui/QTextLine-cursorToX-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.descent
        :returns:
            float
        :description: QtGui/QTextLine-descent-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.draw
        :args:
            :sip:ref:`~PyQt5.QtGui.QPainter`
            Union[:sip:ref:`~PyQt5.QtCore.QPointF`, :sip:ref:`~PyQt5.QtCore.QPoint`]
            selection: :sip:ref:`~PyQt5.QtGui.QTextLayout.FormatRange` = None
        :description: QtGui/QTextLine-draw-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.glyphRuns
        :args:
            from: int = -1
            length: int = -1
        :returns:
            List[:sip:ref:`~PyQt5.QtGui.QGlyphRun`]
        :description: QtGui/QTextLine-glyphRuns-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.height
        :returns:
            float
        :description: QtGui/QTextLine-height-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.horizontalAdvance
        :returns:
            float
        :description: QtGui/QTextLine-horizontalAdvance-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.isValid
        :returns:
            bool
        :description: QtGui/QTextLine-isValid-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.leading
        :returns:
            float
        :description: QtGui/QTextLine-leading-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.leadingIncluded
        :returns:
            bool
        :description: QtGui/QTextLine-leadingIncluded-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.lineNumber
        :returns:
            int
        :description: QtGui/QTextLine-lineNumber-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.naturalTextRect
        :returns:
            :sip:ref:`~PyQt5.QtCore.QRectF`
        :description: QtGui/QTextLine-naturalTextRect-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.naturalTextWidth
        :returns:
            float
        :description: QtGui/QTextLine-naturalTextWidth-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.position
        :returns:
            :sip:ref:`~PyQt5.QtCore.QPointF`
        :description: QtGui/QTextLine-position-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.rect
        :returns:
            :sip:ref:`~PyQt5.QtCore.QRectF`
        :description: QtGui/QTextLine-rect-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.setLeadingIncluded
        :args:
            bool
        :description: QtGui/QTextLine-setLeadingIncluded-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.setLineWidth
        :args:
            float
        :description: QtGui/QTextLine-setLineWidth-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.setNumColumns
        :args:
            int
        :description: QtGui/QTextLine-setNumColumns-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.setNumColumns
        :args:
            int
            float
        :description: QtGui/QTextLine-setNumColumns-f-1.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.setPosition
        :args:
            Union[:sip:ref:`~PyQt5.QtCore.QPointF`, :sip:ref:`~PyQt5.QtCore.QPoint`]
        :description: QtGui/QTextLine-setPosition-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.textLength
        :returns:
            int
        :description: QtGui/QTextLine-textLength-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.textStart
        :returns:
            int
        :description: QtGui/QTextLine-textStart-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.width
        :returns:
            float
        :description: QtGui/QTextLine-width-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.x
        :returns:
            float
        :description: QtGui/QTextLine-x-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.xToCursor
        :args:
            float
            edge: :sip:ref:`~PyQt5.QtGui.QTextLine.CursorPosition` = :sip:ref:`~PyQt5.QtGui.QTextLine.CursorPosition.CursorBetweenCharacters`
        :returns:
            int
        :description: QtGui/QTextLine-xToCursor-f.rst

    .. sip:method:: PyQt5.QtGui.QTextLine.y
        :returns:
            float
        :description: QtGui/QTextLine-y-f.rst

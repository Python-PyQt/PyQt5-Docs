:orphan:

.. sip:class:: PyQt5.QtXml.QDomDocument
    :inherits: :sip:ref:`~PyQt5.QtXml.QDomNode`
    :description: QtXml/QDomDocument-c.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.__init__
        :description: QtXml/QDomDocument-__init__-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.__init__
        :args:
            str
        :description: QtXml/QDomDocument-__init__-f-1.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.__init__
        :args:
            :sip:ref:`~PyQt5.QtXml.QDomDocumentType`
        :description: QtXml/QDomDocument-__init__-f-2.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.__init__
        :args:
            :sip:ref:`~PyQt5.QtXml.QDomDocument`
        :description: QtXml/QDomDocument-__init__-f-3.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.createAttribute
        :args:
            str
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomAttr`
        :description: QtXml/QDomDocument-createAttribute-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.createAttributeNS
        :args:
            str
            str
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomAttr`
        :description: QtXml/QDomDocument-createAttributeNS-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.createCDATASection
        :args:
            str
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomCDATASection`
        :description: QtXml/QDomDocument-createCDATASection-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.createComment
        :args:
            str
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomComment`
        :description: QtXml/QDomDocument-createComment-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.createDocumentFragment
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomDocumentFragment`
        :description: QtXml/QDomDocument-createDocumentFragment-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.createElement
        :args:
            str
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomElement`
        :description: QtXml/QDomDocument-createElement-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.createElementNS
        :args:
            str
            str
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomElement`
        :description: QtXml/QDomDocument-createElementNS-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.createEntityReference
        :args:
            str
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomEntityReference`
        :description: QtXml/QDomDocument-createEntityReference-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.createProcessingInstruction
        :args:
            str
            str
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomProcessingInstruction`
        :description: QtXml/QDomDocument-createProcessingInstruction-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.createTextNode
        :args:
            str
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomText`
        :description: QtXml/QDomDocument-createTextNode-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.doctype
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomDocumentType`
        :description: QtXml/QDomDocument-doctype-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.documentElement
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomElement`
        :description: QtXml/QDomDocument-documentElement-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.elementById
        :args:
            str
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomElement`
        :description: QtXml/QDomDocument-elementById-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.elementsByTagName
        :args:
            str
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomNodeList`
        :description: QtXml/QDomDocument-elementsByTagName-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.elementsByTagNameNS
        :args:
            str
            str
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomNodeList`
        :description: QtXml/QDomDocument-elementsByTagNameNS-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.implementation
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomImplementation`
        :description: QtXml/QDomDocument-implementation-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.importNode
        :args:
            :sip:ref:`~PyQt5.QtXml.QDomNode`
            bool
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomNode`
        :description: QtXml/QDomDocument-importNode-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.nodeType
        :returns:
            :sip:ref:`~PyQt5.QtXml.QDomNode.NodeType`
        :description: QtXml/QDomDocument-nodeType-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.setContent
        :args:
            Union[:sip:ref:`~PyQt5.QtCore.QByteArray`, bytes, bytearray]
        :returns:
            bool
            str
            int
            int
        :description: QtXml/QDomDocument-setContent-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.setContent
        :args:
            str
        :returns:
            bool
            str
            int
            int
        :description: QtXml/QDomDocument-setContent-f-1.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.setContent
        :args:
            :sip:ref:`~PyQt5.QtCore.QIODevice`
        :returns:
            bool
            str
            int
            int
        :description: QtXml/QDomDocument-setContent-f-2.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.setContent
        :args:
            Union[:sip:ref:`~PyQt5.QtCore.QByteArray`, bytes, bytearray]
            bool
        :returns:
            bool
            str
            int
            int
        :description: QtXml/QDomDocument-setContent-f-3.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.setContent
        :args:
            str
            bool
        :returns:
            bool
            str
            int
            int
        :description: QtXml/QDomDocument-setContent-f-4.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.setContent
        :args:
            :sip:ref:`~PyQt5.QtCore.QIODevice`
            bool
        :returns:
            bool
            str
            int
            int
        :description: QtXml/QDomDocument-setContent-f-5.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.setContent
        :args:
            :sip:ref:`~PyQt5.QtXml.QXmlInputSource`
            bool
        :returns:
            bool
            str
            int
            int
        :description: QtXml/QDomDocument-setContent-f-6.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.setContent
        :args:
            :sip:ref:`~PyQt5.QtXml.QXmlInputSource`
            :sip:ref:`~PyQt5.QtXml.QXmlReader`
        :returns:
            bool
            str
            int
            int
        :description: QtXml/QDomDocument-setContent-f-7.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.setContent
        :args:
            :sip:ref:`~PyQt5.QtCore.QXmlStreamReader`
            bool
        :returns:
            bool
            str
            int
            int
        :description: QtXml/QDomDocument-setContent-f-8.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.toByteArray
        :args:
            indent: int = 1
        :returns:
            :sip:ref:`~PyQt5.QtCore.QByteArray`
        :description: QtXml/QDomDocument-toByteArray-f.rst

    .. sip:method:: PyQt5.QtXml.QDomDocument.toString
        :args:
            indent: int = 1
        :returns:
            str
        :description: QtXml/QDomDocument-toString-f.rst

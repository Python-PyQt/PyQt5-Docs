...
The :py:mod:`pyqtbuild` module implements the PyQt build system.  This is an
extension of the SIP build system implemented by the :py:mod:`sipbuild` module.
To install it, together with the :program:`pyqt-bundle` program, from PyPI,
run::

    pip install PyQt-builder

If you are only building PyQt, or one of its related projects, then you do not
...
This will ensure that ``PyQt-builder`` is automatically installed when a user
...
.. py:class:: PyQtProject(\*\*kwargs)
    ...
    - the default value of ``sip-module`` is :sip:ref:`PyQt5.sip`.

# PyQtDocs - The PyQt Documentation
...
The `conf.py` file in the `docs` directory should be updated with the X.Y.Z
version number of the new PyQt version.

For each PyQt module (e.g. QtCore), run the following command:

    python sip2rst.py --package PyQt5 module

This will update the files in the `api` sub-directory of the `docs` directory.

The above command assumes that the PyQt .sip files have been installed in their
default location and that the `sip` executable is on `PATH`.  `sip2rst.py` has
command line options to override the default locations if necessary.


## Re-basing for New Versions of Qt

To update the descriptions of the PyQt API in the `descriptions` directory for
a new version of Qt, run the following command:

    python webxml2rst.py --package PyQt5 --qdoc QDOC --qt-source SRC

`QDOC` is the pathname of the `qdoc` executable.
